...
function PhotoVideoGalleryMasterShortCode( $Id ) {
    ...
    if(!isset($AllGalleries['post_id'])) {
		...
		$PVGM_Gallery_Settings = "PVGM_Gallery_Settings_".$PVGM_Id;
		$PVGM_Gallery_Settings = unserialize(get_post_meta( $PVGM_Id, $PVGM_Gallery_Settings, true));
	...
	<?php
		...
		if($TotalImages) {
			foreach($PVGM_AllPhotosDetails as $PVGM_SinglePhotoDetails) {
				...
				$description = esc_textarea( $PVGM_SinglePhotoDetails['PVGM_image_descp'] );
				if($type=="image"){
					$href_link = $url;
				}  elseif($type=="link"){
					$href_link = $link;
